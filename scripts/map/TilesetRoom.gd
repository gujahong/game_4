...
const RECORDS := [
	Vector2(-15.0, -2.2),   # 서쪽 열람실 - 안쪽 벽
	Vector2(-12.5, 2.2),    # 서쪽 열람실 - 문 가까이
	Vector2(15.0, 2.2),     # 동쪽 열람실 - 안쪽 벽
	Vector2(12.5, -2.2),    # 동쪽 열람실 - 문 가까이
]
## 서가 앞에 서면 반응하는 거리(칸).
...
const SLEEPERS := [
	Vector2(1.1, 8.0),     # 남쪽 복도 - 들어와서 처음 만난다
	Vector2(-8.0, -1.1),   # 서쪽 복도 - 열람실 가는 길
	Vector2(8.0, 1.1),     # 동쪽 복도 - 열람실 가는 길
	Vector2(4.2, -4.2),    # 홀 북동 구석 - 그것에게 가는 문 앞
]
