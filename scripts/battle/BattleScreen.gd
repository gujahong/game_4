...
func _capture_brightness_and_quit() -> void:
	...
	# 밝기 칸은 눈금에서 파생된다. 칸마다 눈금을 그 구간 값으로 직접 놓는다.
	# **여섯 칸이다**(2026-08-21) — 환함·밝음·은은함·어스름·불씨·꺼짐.
	var bands: Array = [10, 8, 6, 4, 2, 0]   # 환함 -> 꺼짐
	for step in bands.size():
		var index: int = bands.size() - 1 - step
		battle.lantern.marks = bands[step]
		battle.lantern.mana = bands[step]
		battle.state_changed.emit()
		await _shoot("res://tools/_battle_%d.png" % index)

	# CRT를 끈 것과 켠 것. 어느 쪽이 나은지 눈으로 고르려고 같은 장면을 두 번 찍는다.
	battle.lantern.marks = 4
	battle.lantern.mana = 3
	battle.state_changed.emit()
