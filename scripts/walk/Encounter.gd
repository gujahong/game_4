...
func _ready() -> void:
	...
	# 적마다 다른 데서 싸우면 그 얼굴이 흐려진다. 다만 8초를 걷고 붙잡히는 연출은 그것에게만
	# 쓴다(`straight`).
	...
	if straight or "--battle" in OS.get_cmdline_user_args():
		...
		# 떠 있는 고리(그것)의 것이다 - 바닥에 선 것이 기울어 있으면 수평이 안 맞는다.
...
func _begin_battle() -> void:
	...
	# **일렁임은 그것에게만 걸린다.** 픽셀을 바깥으로 밀어내는 셰이더라, 다른 그림에 그대로
