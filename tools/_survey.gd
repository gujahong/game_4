...
const LIST := [
	...
	"res://assets/enemies/paper.png",
	"res://assets/enemies/paper_pile.png",
	"res://assets/enemies/watcher.png",
	"res://assets/enemies/hollow_armour.png",
	...
	for p in LIST:
		...
		for y in img.get_height():
			for x in img.get_width():
				...
				sat += c.s
				n += 1
		print("%-46s %4dx%-4d  색 %4d개  채도 %.2f" % [
			p.get_file(), img.get_width(), img.get_height(), seen.size(),
			sat / maxf(float(n), 1.0)])
	quit()
